 ...
 */
const STROKE_WIDTH: any = 20;
const joy = '<svg\n  class="act-page-loading__joy"\n  viewBox="'
  .concat((-STROKE_WIDTH).toString(), ' ')
  .concat((-STROKE_WIDTH).toString(), ' ')
  .concat((555).toString() + STROKE_WIDTH * 2, ' ')
  .concat(
    (435).toString() + STROKE_WIDTH * 2,
 ...
 */
let count: number = 0; //显示计数

let dom: any = null; //页面loading的dom

let hiding: boolean = false; //是否正在隐藏
